...
final class SortIndexPerformanceTests: XCTestCase {
    ...
    func test_NoIndex_SortPerformance() throws {
        let queries: QueryGroup<TestingModels.NotIndexed> = TestingModels.NotIndexed.query(in: context)
        measure {
            let _ = queries
                .sorted(by: \.numOf1)
                .resolve()
    ...
    func test_Indexed_SortPerformance() throws {
        let queries: QueryGroup<TestingModels.SingleValueIndexed> = TestingModels.SingleValueIndexed.query(in: context)
        measure {
            let _ = queries
                .sorted(by: \.numOf1)
                .resolve()
    ...
    func test_EvalProperyIndexed_SortPerformance() throws {
        let queries: QueryGroup<TestingModels.EvaluatedPropertyDescIndexed> = TestingModels.EvaluatedPropertyDescIndexed.query(in: context)
        measure {
            let _ = queries
