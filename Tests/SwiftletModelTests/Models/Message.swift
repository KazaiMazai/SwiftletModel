...
extension Schema.V1 {
    ...
    struct Message: Codable, Sendable {
        ...
        @Index(\Message.timestamp) private var timestampIndex
        @FullTextIndex(\Message.text) private var textSearchIndex1
...
extension Query<Message> {
    func isMyMessage(in context: Context) -> Bool? {
        related(\.$author)
            .resolve(in: context)?.isCurrent == true
    }
}
